...
def dotproduct(v1,v2):
  #print v1,v2,sum([v1[i]*v2[i] for i in range(len(v1))])
  #for i in range(len(v1)):
  #  print v1[i],v2[i],v1[i]*v2[i]
...
def scaledata(rows):
  ...
  for row in rows:
    ...
    if low[i]==high[i]: #w wz125
      low[i]=-low[i]
  # Create a function that scales data
  def scaleinput(d):
     #print d,len(low),(d[7]-low[7]),high[7],low[7]
...
def nlclassify(point,rows,offset,gamma=10):
  ...
  count0=0
  count1=0
  ''' 计算point 与rows中所有数据（0 1）的距离平均值
      point 是经过scaleinput 缩放的，保持与rows一致
  ''' 
  for row in rows:
    if row.match==0:
